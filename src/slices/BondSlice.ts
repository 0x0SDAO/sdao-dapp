...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, address, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    if (!value || value === "") {
      value = "0";
    }
    ...
    try {
      debtRatio = Number((await bondContract.standardizedDebtRatio()).toString()) / Math.pow(10, 9);
    } catch (e) {
...
export const redeemBond = createAsyncThunk(
  ...
  async ({ address, bond, networkID, provider, autostake }: IRedeemBondAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      const pendingTxnType = "redeem_bond_" + bond + (autostake ? "_autostake" : "");
